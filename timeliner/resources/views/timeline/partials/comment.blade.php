<div class="card mb-3">
    ...
    <div class="card-body">
        ...
        <button class="btn btn-primary bi bi-pencil" onclick="document.getElementById('editComment{{ $comment->id }}').classList.toggle('hidden')"> Edit Comment</button>
        <form id="editComment{{ $comment->id }}" class="hidden" action="{{ route('comment.update', $comment->id) }}" method="POST">
            ...
            <button type="submit" class="btn btn-secondary bi bi-check2"> Confirm Edit</button>
        </form>
